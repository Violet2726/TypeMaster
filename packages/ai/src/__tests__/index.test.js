...
describe('ai-service', () => {
    ...
    describe('AI proxy availability', () => {
        ...
        it('should enable AI proxy when the env flag is explicit', () => {
            vi.stubEnv('NEXT_PUBLIC_TYPEMASTER_AI_PROXY', '1');
    ...
    describe('generatePracticeText', () => {
        it('should call the practice-text product endpoint and create a draft', async () => {
            vi.stubEnv('NEXT_PUBLIC_TYPEMASTER_AI_PROXY', '1');
            ...
            const draft = await generatePracticeText({
                mode: 'words',
                durationSeconds: 60,
                wordCount: 4,
                ...
                difficulty: 'easy',
                source: 'ai'
            }, 'warm up', { language: 'en-US' });

            expect(fetchMock).toHaveBeenCalledWith('/api/practice-text', expect.objectContaining({
                method: 'POST'
            }));
            expect(JSON.parse(fetchMock.mock.calls[0][1].body)).toMatchObject({
                promptOverride: 'warm up',
                language: 'en-US'
            });
            expect(draft.words).toEqual(['alpha', 'beta', 'gamma', 'delta']);
            expect(draft.sourceTextMeta.generatedBy).toBe('ai');
        });

        it('should build provider payloads without exposing provider details to the page layer', () => {
            const payload = buildPracticeTextProviderPayload({
                mode: 'words',
                durationSeconds: 60,
                wordCount: 8,
                includePunctuation: false,
                ...
                difficulty: 'easy',
                source: 'ai'
            }, 'short rhythm drill', { language: 'en-US' });

            expect(payload.stream).toBe(false);
            expect(payload.messages[0].role).toBe('system');
            expect(payload.messages[1].content).toContain('short rhythm drill');
        });
    ...
    describe('generateCoachAdvice', () => {
        it('should call the coach product endpoint and normalize advice', async () => {
            vi.stubEnv('NEXT_PUBLIC_TYPEMASTER_AI_PROXY', '1');
            const fetchMock = vi.fn().mockResolvedValue(new Response(JSON.stringify({
                advice: {
                    headline: 'Good round',
                    ...
                    nextDrill: {
                        ...
                        reason: 'Slow down slightly.',
                        configPatch: { difficulty: 'easy' },
                        aiPrompt: 'Generate accuracy text.'
                    },
                    comparison: {
                        label: 'steady',
                        summary: 'Stable.'
            ...
            const advice = await generateCoachAdvice({
                session: { id: 'session-1', config: {}, result: {}, sourceTextMeta: {} },
                history: [],
                language: 'en-US'
            });
        ...
        it('should build coach provider payloads for the API route', () => {
            const payload = buildCoachAdviceProviderPayload({
                session: {
                    id: 'session-1',
                    config: { aiTemplate: 'daily' },
                    result: { wpm: 80, accuracy: 97 },
                    sourceTextMeta: { source: 'ai' }
                },
                history: [{ id: 'session-0', result: { wpm: 70, accuracy: 95 } }],
                language: 'en-US'
            });

            expect(payload.stream).toBe(false);
            expect(payload.response_format).toEqual({ type: 'json_object' });
            expect(payload.messages[1].content).toContain('recentHistory');
        });
