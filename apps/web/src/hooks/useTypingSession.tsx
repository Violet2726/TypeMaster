...
import { calculateMetrics, createTimelinePoint, createEmptyTimeline, computeElapsedMs, commitWord, handleBackspace, calculatePauseSecond, shouldAddPauseMoment, computePausedDuration, computeTimerDisplay } from '@typemaster/domain';
import { playTypingSound } from '../services/sound';

type FinishSessionOptions = {
    forcedElapsedMs?: number | null,
    finalHistoryOverride?: string[] | null,
};
...
export function useTypingSession({ draft, config, soundEffects = false, onComplete }) {
    ...
    const finishSession = useCallback((options: FinishSessionOptions = {}) => {
        ...
        const interval = window.setInterval(() => {
            ...
            if (nextSecond > lastHistorySecondRef.current) {
                ...
                setTimeline((previous) => ({
                    ...
                    pauseMoments: previous.pauseMoments
                }));
