...
import { getTypeRiftScore, getTypeRiftSessions } from '../game-session';
import type { CountStat, TrainingSession } from '../../../types/training';
...
export function useGameStore() {
    ...
    const keyboardHotspots = useMemo(() => {
        ...
        recentSessions.forEach((session: TrainingSession) => {
            ...
            if (Array.isArray(charStats)) {
                charStats.forEach((stat: CountStat) => {
    ...
    const gameSessions = useMemo(() => getTypeRiftSessions(sessions || []), [sessions]);
    const gameBestScore = useMemo(() => Math.max(0, ...gameSessions.map(getTypeRiftScore)), [gameSessions]);
    const gameCodex = useMemo(() => buildGameCodexFromSessions(gameSessions), [gameSessions]);
